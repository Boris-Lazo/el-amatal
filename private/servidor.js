...
(async () => {
  try {
    ...
    app.use(
      helmet({
        contentSecurityPolicy: false, // Desactivar CSP por ahora para evitar problemas con iframes externos (Google Maps)
      })
    );
    app.use(compression());
    app.use(morgan('dev'));
    ...
    // Opciones para archivos estáticos con control de caché
    const opcionesEstaticos = {
      maxAge: '1d',
      setHeaders: (res, ruta) => {
        if (ruta.endsWith('.html')) {
          // No cachear HTML para asegurar que el SPA cargue la versión más reciente
    ...
    app.use(
      express.static(path.join(__dirname, '..', 'dist'), opcionesEstaticos)
    );
    app.use(
      express.static(path.join(__dirname, '..', 'public'), opcionesEstaticos)
    );

    // Servir fotos de álbumes y documentos
    app.use('/api/subidas', express.static(configuracionApp.rutas.albumes));
    app.use(
      '/api/documentos/archivo',
      express.static(configuracionApp.rutas.documentos)
    );
    ...
    app.use((err, req, res, next) => {
      if (err instanceof multer.MulterError) {
        ...
        return res
          .status(400)
          .json({ error: `Error al subir archivo: ${err.message}` });
      } else if (err) {
    ...
    const servidor = app.listen(PUERTO, () =>
      console.log(
        `✅ Backend con arquitectura SOLID en http://localhost:${PUERTO}`
      )
    );
