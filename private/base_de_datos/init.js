  ...
  const db = new sqlite.Database(archivoDb, (err) => {
    ...
    db.serialize(() => {
      ...
      const predeterminados = [
        {
          usuario: process.env.USER_DIRECTORA_EMAIL,
          clave: process.env.USER_DIRECTORA_PASS,
        },
        {
          usuario: process.env.USER_SUBDIRECTORA_EMAIL,
          clave: process.env.USER_SUBDIRECTORA_PASS,
        },
        {
          usuario: process.env.USER_DEV_EMAIL,
          clave: process.env.USER_DEV_PASS,
        },
      ].filter((u) => u.usuario && u.clave);

      if (predeterminados.length > 0) {
        const sentencia = db.prepare(
          'INSERT OR IGNORE INTO users (user, hash) VALUES (?, ?)'
        );
