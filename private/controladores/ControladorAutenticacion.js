...
class ControladorAutenticacion {
  ...
  async iniciarSesion(req, res, next) {
    try {
      ...
      const token = await this.servicioAutenticacion.iniciarSesion(
        usuario,
        contrasena
      );
      res.json({ token });
  ...
  async cambiarContrasena(req, res, next) {
    try {
      ...
      await this.servicioAutenticacion.cambiarContrasena(
        correo,
        viejaClave,
        nuevaClave
      );
  ...
  async restablecerContrasena(req, res, next) {
    try {
      ...
      await this.servicioAutenticacion.restablecerContrasena(
        tokenTemporal,
        nuevaClave
      );
