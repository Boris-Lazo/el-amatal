...
class ControladorDocumento {
  ...
  async crearDocumento(req, res, next) {
    try {
      ...
      const resultado = await this.servicioDocumento.crearDocumento(
        { titulo, mes },
        archivo
      );
  ...
  async listarDocumentos(req, res, next) {
    try {
      ...
      const documentos = await this.servicioDocumento.listarDocumentos(limite);
      res.json(documentos);
    } catch (error) {
      next(error);
    }
  ...
  async obtenerMiniatura(req, res, next) {
    try {
      ...
      const rutaMiniatura =
        await this.servicioDocumento.obtenerMiniatura(nombreArchivo);
