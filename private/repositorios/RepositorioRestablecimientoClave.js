...
class RepositorioRestablecimientoClave extends RepositorioBase {
  ...
  async obtenerPorToken(token) {
    return this.obtenerUno('SELECT * FROM password_resets WHERE token = ?', [
      token,
    ]);
  }
  ...
  async eliminarPorToken(token) {
    return this.ejecutar('DELETE FROM password_resets WHERE token = ?', [
      token,
    ]);
  }
  ...
  async eliminarExpirados() {
    return this.ejecutar('DELETE FROM password_resets WHERE expires_at < ?', [
      Date.now(),
    ]);
  }
}
