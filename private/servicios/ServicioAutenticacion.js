...
class ServicioAutenticacion {
  ...
  constructor(
    repositorioUsuario,
    repositorioRestablecimientoClave,
    servicioCorreo
  ) {
  ...
  async solicitarRestablecimiento(correo) {
    console.log('[RECUPERACION] Solicitud de recuperación para:', correo);
    const usuario = await this.repositorioUsuario.obtenerPorCorreo(correo);
    if (!usuario) {
      console.log('[RECUPERACION] Usuario no encontrado:', correo);
      throw new ErrorNoEncontrado('No existe ninguna cuenta asociada a este correo electrónico.');
    }

    const token = Math.floor(100000 + Math.random() * 900000).toString();
    const expiraEn = Date.now() + 15 * 60 * 1000;

    console.log('[RECUPERACION] Token generado:', token);
    await this.repositorioRestablecimientoClave.crear(correo, token, expiraEn);
    console.log('[RECUPERACION] Token guardado en BD');

    try {
      console.log('[RECUPERACION] Intentando enviar correo a:', correo);
      const resultado = await this.servicioCorreo.enviarCodigoRecuperacion(correo, token);
      console.log('[RECUPERACION] ✅ Correo enviado exitosamente:', resultado.messageId);
    } catch (error) {
      console.error('[RECUPERACION] ❌ Error al enviar correo:', error.message);
      console.error('[RECUPERACION] Detalles del error:', error);
  ...
  async restablecerContrasena(token, nuevaClave) {
    const registro =
      await this.repositorioRestablecimientoClave.obtenerPorToken(token);
    if (!registro) {
    ...
    const nuevoHash = await bcrypt.hash(nuevaClave, 10);
    await this.repositorioUsuario.actualizarContrasena(
      registro.user_email,
      nuevoHash
    );
    await this.repositorioRestablecimientoClave.eliminarPorToken(token);
